...
- `results`: Contains outputs generated by project scripts (plots, exported HTML, PNG, CSV metrics). check (results/README.md)
- `dataset`: Contains the datasets used for training and testing (CSV files) and notebooks/scripts for exploring and visualizing the data (e.g. `data_set_eplanation_and_explore.ipynb`).
- `cstm_models`: Custom PyTorch model implementations used in experiments (`mlp.py`, `lstm.py`, `gru.py`, `cnn.py`, `logistic.py`).
- `offline_learn.py`: The main script used to train and test offline models mentioned in our experiment.
- `online_learning.ipynb`: Notebook for online learning.
- `Experiments.ipynb`: Notebook to finalize all our experiments results. 
- `util_fun.py`: Utility helpers functions for the creating sequences, dataset classes, training loops, evaluation, plotting, and metric export.
